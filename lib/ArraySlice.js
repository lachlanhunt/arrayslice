 ...
 * will be reveresed relative to the original object.
 *
 ...
 * value must be a positive unsigned integer, not exceeding 2^32 - 1. Upon setting, the resulting length
 * will be determined such that the new `end` index does not exceed the length of the source array for
 * forward sequences, and does not extend past 0 for reverse sequences.  Changes to the length will
 * have no effect on whether the ArraySlice represents a forward or reverse sequence.
 *
